...
- **Programs** are a Jekyll collection (`_programs/`, configured in `_config.yml` with `permalink: /programs/:name/` and a `defaults` entry setting `layout: program`). `programs.md` is a landing page that generates its card grid from `site.programs | sort: "order"`; `_layouts/program.html` renders each sub-page and derives its own prev/next links from that same sorted list (Jekyll's `page.previous`/`page.next` only work for posts). Adding a file to `_programs/` — with `title`, `order`, `icon`, `card_title`, `summary`, `lead` front matter — adds both a landing card and a stop in the prev/next sequence; no template edits needed. The landing cards carry `id="{{ program.slug }}"` so legacy `/programs#fll` links still land in the right place.
...
- **Shared facts** live in `_data/facts.yml` (stats, ages, costs, schedules) and `_data/season.yml` (competition dates). Any fact appearing on more than one page must be referenced from there via `{{ site.data.facts.* }}` — never hard-coded inline. This includes the registration/wait-list CTA: its wording is `facts.registration.heading`/`.message`, rendered by `_includes/registration-cta.html`, which the programs landing page and every program sub-page include. Because Liquid does not render inside front matter, keep facts in page bodies, not in `_programs/*.md` front matter.
